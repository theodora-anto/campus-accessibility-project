...
- **Υποβολή αναφορών/παραπόνων**: ο φοιτητής μπορεί να υποβάλει παράπονα με τίτλο, κατηγορία, σχολή/τμήμα, τοποθεσία, περιγραφή και έως 5 φωτογραφίες. Μόνο ο admin ξέρει ποιος φοιτητής έχει υποβάλει την αναφορά και σε ποιο τμήμα ανήκει. 
- **Παρακολούθηση κατάστασης**: κάθε αναφορά έχει πλήρες ιστορικό αλλαγών του διαχειριστή.
- **Alt-text workflow**: ο διαχειριστής πρέπει να περιγράψει κάθε φωτογραφία (ή να δηλώσει ρητά ότι δεν χρειάζεται περιγραφή βάζοντας space) πριν μπορέσει να αλλάξει την κατάσταση μιας αναφοράς.
- **Αναζήτηση & φίλτρα**: ο φοιτητής και ο διαχειριστής μπορούν να αναζητούν αναφορές ανά τμήμα, κατηγορία, κατάσταση ή λέξη-κλειδί.
- **Πίνακας διαχείρισης**: ο φοιτητής μπορεί να παρακολουθεί τις δικές του αναφορές. Ο διαχειριστής μπορεί να διαχειρίζεται τις αναφορές ανά κατάσταση και να βλέπει το πλήθος τους ανά κατάσταση.
...
- **`JWT_SECRET`** πρέπει να περιέχει τουλάχιστον 32 χαρακτήρες (στο GitBash παράγεται με την εντολή openssl rand -base64 32)
...
> **Υποσημείωση:** ο φάκελος `campus-accessibility-frontend/` περιέχει επίσης δικό του `.env`/`.env.example` με τα ίδια `VITE_API_URL`/`VITE_API_BASE_URL` keys — **δεν χρειάζεται να τα ρυθμίσετε** για το deployment που περιγράφεται εδώ.
...
## Γνωστοί περιορισμοί - Μελλοντικές επεκτάσεις

 Σε μελλοντική επέκταση το σύστημα θα μπορούσε:
- να υποστηρίζει παραπάνω από έναν admin και το ιστορικό αλλαγών να εκθέτει ποιος συγκεκριμένος admin έκανε κάθε αλλαγή.
- να αποτελεί επιτρέπει αναζήτηση με βάση την αναπηρία πχ. οι φοιτητές να μπορούν να αναζητούν παράπονα που έχουν γίνει μόνο από χρήστες αμαξιδίου. Αυτό απαιτεί διερεύνηση σχετικά με ζητήματα προσωπικών δεδομένων.